...
### Story D.i: v0.5.0 `init` deterministic scaffolder [Done]

`features.md` FR-21. Owns the Phase D v0.5.0 bump.

- [x] Create `src/modelfoundry/scaffolder/__init__.py`, `src/modelfoundry/scaffolder/init.py` with `scaffold_recipe(recipe_path, datarefinery_recipe_path, *, plugin="pytorch", force=False) -> Path`. Reads the bound DataRefinery instance's manifest, picks a baseline architecture / loss / optimizer / training policy / evaluation metrics / OutputExpectations for the dataset shape. (**Signature deviation:** added a keyword `config: RuntimeConfig | None = None` — resolving the DR instance needs the data-cache root; the CLI threads `_config(ctx)`, `None` falls back to `RuntimeConfig()`. Baseline: `num_classes` from `instance_num_classes()`, `in_channels` from the DR record-schema image shape (`shape[-1]`, no decode needed → scaffolder is torch-free), `resnet20` (pytorch) / `mlp_classifier` (sklearn), `cross_entropy` + `adamw`, early-stopping on **`val_loss`** when a val split exists (`val_accuracy` isn't in the validator's check-6 produced-metric vocabulary), eval on the test/val split, and a better-than-chance `accuracy gte round(1/num_classes, 4)` expectation (`OutputExpectations` uses the model's `value` field, not `expected`). Deterministic output bytes.)
- [x] Stamps the Apache-2.0 / Pointmatic header at the top of the recipe as a YAML comment. (`_HEADER` constant prepended; the file starts with `# Copyright (c) 2026 Pointmatic` + the SPDX line + a scaffold-provenance note.)
- [x] CLI: `src/modelfoundry/cli/commands/init_cmd.py` with `modelfoundry init <recipe-path> --data <datarefinery-recipe-path> [--plugin pytorch] [--force]`. (`init_cmd.run` delegates to `scaffold_recipe` and prints the path; `app.py` wires the `recipe` argument + `--data` (required) / `--plugin` / `--force` options.)
- [x] Reserved `scaffolder/llm.py` per the deferred `[llm]` extra — note in the package tree comment that it's not implemented in the pre-production series. (Header-only module documenting the deferred natural-language path; no logic. The package `__init__` docstring notes it too.)
- [x] CLI smoke test: scaffold a recipe against the fixture DataRefinery instance; resulting recipe loads, validates, and materializes (small) cleanly. ([tests/cli/test_init_cmd.py](../../tests/cli/test_init_cmd.py): 13 tests. `resolve_data_instance` is monkeypatched to a synthesized DR instance (8×8 RGB, 3 classes, train/val/test) so scaffold logic runs without DR path-resolution. Covers: loadable pytorch recipe (resnet20 / num_classes 3 / in_channels 3), header stamp, early-stopping `val_loss`, eval split prefers `test`, sklearn `mlp_classifier`, `--force` refuse/overwrite, **scaffolded recipe validates clean** (all 20 checks), **materializes** (1-epoch trim + expectations dropped — a 1-epoch tiny model can't clear better-than-chance), CLI exit 0, missing `--data` → usage error 2.)
- [x] Bump version to v0.5.0. (`_version.py` → `0.5.0`; `modelfoundry --version` prints `modelfoundry 0.5.0`.)
- [x] Update CHANGELOG.md (Phase D summary: full CLI surface — `init`, `validate`, `check`, `status`, `materialize`, `report`, `inspect`, `clean`). (`[Unreleased]` → `[0.5.0] - 2026-06-15` with a Phase D summary paragraph + one bullet per verb (D.c–D.i) added above the previously-recorded D.b/D.a entries.)
- [x] Verify: every CLI verb works against the fixture; `pyve testenv run mypy src tests` clean. (Full suite **512 passed** in `smoke-pytorch`; `ruff check src tests` + `mypy src tests` clean (115 files). `modelfoundry --version` → `0.5.0`.)
