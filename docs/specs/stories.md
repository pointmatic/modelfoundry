...
- [ ] **Failing test first** — `tests/unit/test_pytorch_data.py` (runs in `smoke-pytorch`): build a train `DataLoader` with `num_workers=2` over an instance carrying a lazy `Augmentations` policy under the `spawn` context; iterate one batch → reproduces the pickle `AttributeError`. **red**.
- [ ] Replace the closure with a picklable, module-level callable — a small `_ComposedTransform` class (`__call__(self, img)`) holding the realizer list, or `functools.partial` over a module-level apply function. Preserve exact left-to-right op order and the per-op `derive_seed(master_seed, "augmentation:<name>", salt)` derivation.
- [ ] Verify each `build_realizer` output is itself picklable (no nested lambdas/closures); lift any that are not.
- [ ] **Coverage for the seam** — extend the synthetic `build_dr_instance` builder (`tests/fixtures/datarefinery_instances/builder.py`) to optionally emit a lazy `Augmentations` policy, then add an augmentations × `num_workers ∈ {0, 2}` test asserting success **and** output identity — the exact combination neither `test_determinism` (workers ≥ 1, no augmentations) nor `test_cifar10_resnet20` (augmentations, workers = 0) covered.
- [ ] Bump version to v0.8.5 in [_version.py](../../src/modelfoundry/_version.py).
- [ ] Update CHANGELOG.md (`## [0.8.5]` under Fixed).
- [ ] Verify: the `num_workers=2` augmented loader iterates; output identical to `num_workers=0` (determinism unchanged); `pyve test --env smoke-pytorch` green; `ruff check` + `mypy` clean.
