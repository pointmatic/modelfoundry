...
### Story I.o.2: Clip-level window aggregation engine (R7) + dangling-key refusal + validator [Done]

The consumer-owned aggregation math (DR ships no aggregation op). Layers over the **already-built** MC-dropout per-record outputs ([stochastic.py](../../src/modelfoundry/plugins/pytorch/stochastic.py)), driven by the I.o.1 `WindowAggregation` policy. → New [plugins/pytorch/aggregation.py](../../src/modelfoundry/plugins/pytorch/aggregation.py) (pure-torch grouping + policies + integrity refusal); wired into [evaluation.py](../../src/modelfoundry/plugins/pytorch/evaluation.py); `Plugin.run_evaluation` Protocol gains a `window_aggregation` kwarg threaded base + pytorch/sklearn(+random) + runner; new validator check 23.

- [x] **Group + aggregate.** Regroup window-level predictions (incl. MC-dropout means / `predictive_entropy` / `mc_variance`) by `source_record_id` into clip-level results; apply the declared policy. All three policies produce a clip-level `(C,)` probability vector (mean of window probs / logit-space mean / normalized vote histogram) so clip-level metrics + uncertainty compute uniformly. `window_index` available for order-sensitive policies. Thread `recipe.WindowAggregation` through the `Plugin.run_evaluation` Protocol (base + pytorch/sklearn/random) into the evaluation stage; emit a clip-level predictions surface + clip-level metrics when present. → `aggregate_windows` groups members in first-appearance clip order, members ordered by `window_index`; `mean` / `logit_average` (renormalized geometric mean) / `majority_vote` (normalized argmax histogram) in [aggregation.py](../../src/modelfoundry/plugins/pytorch/aggregation.py). [evaluation.py](../../src/modelfoundry/plugins/pytorch/evaluation.py) regroups per split when `window_aggregation` present (clip target = first window's; clip MC uncertainty = entropy of the clip distribution + mean per-window `mc_variance`); predictions + metrics become clip-level (`record_id = source_record_id`). `RandomPlugin` inherits the sklearn no-op wrapper. New [DataRefineryDataset.window_keys()](../../src/modelfoundry/plugins/pytorch/data.py#L187). Tests: [test_pytorch_window_aggregation.py](../../tests/unit/test_pytorch_window_aggregation.py) (policy math + grouping), [test_pytorch_audio_evaluation.py](../../tests/unit/test_pytorch_audio_evaluation.py) (clip-level vs window-level surface).
- [x] **Dangling-key failure mode.** Per vendor-spec § Failure modes: a window whose `source_record_id` does not resolve to its clip — DR guarantees `record_id == f"{source_record_id}__w{window_index:04d}"`, so a mismatch (or missing `source_record_id`) → **refuse**. Test via the I.l dangling-key fixture variant. → `verify_window_integrity` ([aggregation.py](../../src/modelfoundry/plugins/pytorch/aggregation.py)) runs before grouping and raises `DataBindingError`; covered against the I.l `dangling_source_record_id` fixture in [test_pytorch_audio_evaluation.py](../../tests/unit/test_pytorch_audio_evaluation.py) (`test_dangling_source_record_id_refused_during_aggregation`).
- [x] **Validator cross-check.** When `WindowAggregation` is declared, cross-check the bound instance's records carry `source_record_id` (a producible grouping); surface a misdeclaration at `validate`, not mid-run. → New **check 23** (`window_aggregation_grouping`) in [validator.py](../../src/modelfoundry/recipe/validator.py) — torch-free, peeks one on-disk record, skip-with-pass when indeterminate; fails when `WindowAggregation` is declared but records lack `source_record_id`. Check count 22 → 23 (tests + the `range(1, 24)` assertions in [test_recipe_validator.py](../../tests/unit/test_recipe_validator.py); the doc-side "checks 1..22" refresh rides I.r). Full CI gate green: ruff + ruff format + mypy (169) + light (590 passed / 51 skipped / 1 xfailed) + smoke-pytorch (822 passed / 15 skipped / 1 xfailed).
...
### Story I.r: Doc sync, project-essentials append & release — owns the bump (→ v0.18.0) [Done]

- [x] **`features.md`** — add FR-AUDIO-1 (feature-array consumption), FR-AUDIO-2 (clip-level window aggregation R7 + dangling-key refusal), FR-AUDIO-3 (reproducibility parity). → Added the three FRs (Behavior + Edge Cases) after FR-27; enumerated **check 23** (`WindowAggregation` grouping key) in FR-2; TR-3 "checks 1..22 → 1..23"; added the `WindowAggregation` block to the Inputs recipe-shape list.
- [x] **`tech-spec.md`** — reflect the loader feature-array branch, the `audio_normalize` mel-axis read path, the window/clip record model, and the aggregation-policy recipe field. → Module tree gains `stochastic.py` + `aggregation.py`; `data.py` description gains the feature-array branch; `evaluation.py` gains the clip-level wiring + a new `aggregation.py` entry; `ModelRecipe` snippet gains `Inference` + `WindowAggregation`; `Plugin.run_evaluation` signature gains `inference` + `window_aggregation`; validator docstring + test-tree "1..22 → 1..23".
- [x] **`concept.md` / `README.md`** — adjust scope wording if needed (audio feature consumption now in scope on the PyTorch plugin). → `concept.md` In-scope gains an audio feature-array-consumption bullet (a data-consumption capability, no new architecture — stays within NG-7). **`README.md`: deliberately left** — it is quickstart/example-driven with no bundled audio recipe, so scope prose without a runnable example would mislead; the I.q README edit already added the encoder section.
- [x] **Vendor-spec mirror note** — record that MF's consumer half is **ready**. → The persistence half was already reconciled to **shipped/verified** (DR v0.25.0 + I.m.1) in the mirror; added an **MF-consumer-status note** to § "Aggregation contract (R7)" recording the clip-aggregation half shipped (I.o.2: `aggregation.py`, `verify_window_integrity`, check 23) + the I.p acceptance, flagged as a consumer annotation (not a shared-contract change). *(DR has since shipped, so "forward-declared until DR ships" is moot — the seam is shipped both sides.)*
- [x] **`project-essentials.md`** — append any new must-know facts (plan_phase Step 8): the `feature_path`/`audio_normalize`/window-aggregation loader contract and the read-only loose-coupling reminder. → New section "**Audio feature-array consumption is a read-only, additive loader capability**" (branch selection / Q1 anchor / mel-axis `audio_normalize` / evaluation-stage aggregation / sparse-section byte-neutrality / read-only loose-coupling + the tempting-mistakes list). Also corrected a **stale fact** in the determinism section (`num_workers` described as a recipe field — it is execution context since I.e.1).
- [x] **Release** — owns the single minor bump **→ v0.18.0**. → `_version.py` `0.17.1 → 0.18.0`; `CHANGELOG.md` `## [0.18.0] - 2026-06-23` entry (audio feature-array consumption; **not cache-invalidating** — additive loader + sparse `WindowAggregation` segment). Full CI gate green: ruff + ruff format --check + mypy (170) + light (590 passed / 52 skipped / 1 xfailed, incl. `test_release_metadata` CHANGELOG↔version guard at 0.18.0) + smoke-pytorch (826 passed / 15 skipped / 1 xfailed).
...
### Story I.t: FR-12 sklearn-fit baseline resolver + scoring + check-13 [Done]

Implement the resolver from I.s, replacing the warn-and-skip stub at [evaluation.py:421-426](../../src/modelfoundry/plugins/pytorch/evaluation.py#L421); close the FR-12 promise (`concept.md` `comparison_baseline_gap`, `features.md` FR-12) for the sklearn half. → New [plugins/sklearn/baseline.py](../../src/modelfoundry/plugins/sklearn/baseline.py) (grammar + allowlist + seeded fit-on-train scorer); wired into [evaluation.py](../../src/modelfoundry/plugins/pytorch/evaluation.py); check 13 tightened; report gains a `## Comparison` subsection.

- [x] **Resolver + fit-on-train.** Parse `baseline_model_id` per the I.s grammar; resolve the sklearn estimator class; instantiate with `random_state` from `derive_seed(seed, "baseline")`; fit on the `train` feature matrix via the C.f flattening path. → `score_baseline()` parses `sklearn:<EstimatorClassName>` (`parse_baseline_model_id`), resolves via the curated `ALLOWLIST` (RandomForest/GradientBoosting/LogisticRegression/KNeighbors/Dummy), `_instantiate_seeded` sets `random_state = derive_seed(seed,"baseline") & _U32` only when the estimator accepts one (KNN guard), fits on `feature_matrix(data,"train")` (the C.f path).
- [x] **Score + persist.** Score the fitted baseline on every `Evaluation.splits` entry with the same metric set; write results under `evaluation/metrics.json` `baseline.<split>.<metric>`; surface them in the report's comparison subsection. `.evaluation` reads them back with no second scoring path. → `score_baseline` returns `{split: {metric}}` (reusing the new shared `metrics.score_split` + cm/curve); the pytorch evaluator folds it in as `metrics["baseline"]` → `metrics.json` `baseline.*`. Report `_comparison_section` renders main-vs-baseline; `_metrics_section` excludes the `baseline` key from the per-split table. `.evaluation` reads `metrics.json` directly (no second scoring path).
- [x] **Tighten check 13 + flip the xfail.** Enforce the grammar in validator check 13 ([validator.py:361](../../src/modelfoundry/recipe/validator.py#L361)); flip `test_invalid_baseline_model_id_fixture_should_trip_check_13` from `xfail(strict=True)` to a pass ([test_recipe_validator.py:795](../../tests/unit/test_recipe_validator.py#L795)). → Check 13 now calls `parse_baseline_model_id` (format-only; allowlist membership is a runtime concern); the `xfail` removed (now passes) + added a well-formed-passes companion test. Updated the shared good-recipe builder's `baseline_model_id` (`hf://…` → `sklearn:RandomForestClassifier`).
- [x] **Tests.** A comparison-declaring recipe produces `baseline.*` metrics; **byte-identical across two runs** (seeded baseline); a malformed id trips check 13; the unresolvable-but-well-formed path still warns + omits + lets main metrics proceed. → [test_sklearn_baseline_comparison.py](../../tests/unit/test_sklearn_baseline_comparison.py) (grammar/resolution/scoring/byte-identity/unknown-class) + [test_pytorch_evaluation.py](../../tests/integration/test_pytorch_evaluation.py) (resolves+writes `baseline.*`, byte-identical, unresolvable-warns-and-omits) + [test_reporting.py](../../tests/unit/test_reporting.py) (`## Comparison` render + baseline excluded from metrics table). Factored `metrics.score_split` shared with the sklearn plugin. Full CI gate green: ruff check + ruff format --check (172) + mypy (172) + light (594 passed / 53 skipped) + smoke-pytorch.
...
### Story I.u: FR-3 validate-before-materialize [Done]

Honor FR-3 Behavior step 1 ("Run `validate` (FR-2); fail fast on any failure") — today [`ModelFoundry.materialize()`](../../src/modelfoundry/core/modelfoundry.py#L189) constructs `MaterializeRunner().run()` directly with no validation, and the runner does not validate either. → `materialize()` now runs `self.validate()` first (raises `ValidationError` on any failure), then a constant-time cache-identity check, before constructing the runner.

- [x] **Validate first.** Run the FR-2 checks at the head of `materialize()` (library) so both the library and CLI surfaces fail fast before any temp dir / pipeline work. Raise the existing `ValidationError` on any failure. → [modelfoundry.py:`materialize`](../../src/modelfoundry/core/modelfoundry.py#L189) calls `self.validate()`; `not report.passed` → `ValidationError` naming the failed `check N (name)` list + a `detail.failures` dump, raised **before** any `MaterializeRunner`/temp-dir work. The CLI inherits it (drives the same method).
- [x] **Preserve the constant-time cache-hit path.** Settle the ordering (FR-3 favors validate-then-cache-identity; keep the hit short-circuit cheap). Document the chosen order in the story. → **Ordering decided: validate-first (FR-3 step 1) → constant-time cache-identity (step 2) → pipeline.** Validation is a cheap static pass that always runs first (even on would-be hits). The cache-identity check (`instance_dir/manifest.json` exists + `not overwrite`) now short-circuits to the **shipped refusal** (`ModelArtifactExistsError`) **before the pipeline runs** — replacing the prior wasteful "full pipeline then refuse at promote." **Note: features.md FR-3 line 250 says hit⇒return-existing, but the shipped pre-prod contract is hit⇒refuse ([test_loose_coupling.py](../../tests/integration/test_loose_coupling.py)); preserved the shipped refusal and flagged the doc divergence for I.y** (not an I.u contract change). `--overwrite` suppresses the hit (runner trashes + rebuilds, unchanged).
- [x] **Tests.** An invalid recipe raises before any `.tmp/<run-id>/` directory is created; a valid recipe is unaffected; a cache hit still short-circuits without re-running the full pipeline. → [test_modelfoundry_api.py](../../tests/integration/test_modelfoundry_api.py): `test_materialize_validates_first_and_raises_before_temp_dir` (invalid `primary_metric` → `ValidationError`, no `.tmp/` dir) + `test_materialize_cache_hit_short_circuits_without_rerunning` (second materialize → `ModelArtifactExistsError`, no new temp dir). **Fixture correction:** two inline materialize fixtures ([test_modelfoundry_api.py](../../tests/integration/test_modelfoundry_api.py), [test_materialize_cmd.py](../../tests/cli/test_materialize_cmd.py)) carried `[0,1]`-scale `normalize` means that (correctly) trip the new validate-first run's **check 21** (H.a units guard) — bumped to 0-255-scale `[120,110,100]`/`[60,55,50]` to mirror the shared builder. Full CI gate green: ruff + ruff format --check (172) + mypy (172) + light (594 passed / 53 skipped) + smoke-pytorch.

**Version:** no bump (rides v0.19.0). Behavior-only; no canonical-bytes or output-bytes change for any run that already succeeded.

---

### Story I.v: FR-10 NaN-divergence hard error [Done]

Honor the FR-10 edge case ("Training diverges (loss becomes NaN) → hard error with a clear 'training diverged at epoch N' message; partial history preserved; `FAILED` marker written") — [trainer.py](../../src/modelfoundry/plugins/pytorch/trainer.py) has no non-finite-loss guard today. → Added a `math.isfinite(train_loss)` guard in the per-epoch loop ([trainer.py](../../src/modelfoundry/plugins/pytorch/trainer.py)).

- [x] **Divergence guard.** In the training loop, on `not torch.isfinite(loss)` raise `PluginError("training diverged at epoch N")` naming the epoch. The partial `training/history.parquet` is already in the temp dir; the atomic layer ([cache/atomic.py](../../src/modelfoundry/cache/atomic.py)) writes `FAILED` on the raised exception. → After each epoch's `record` is appended, `if not math.isfinite(train_loss)`: write the partial history (including the diverged epoch) then `raise PluginError("training diverged at epoch N: train_loss is non-finite (…)", stage="training")`. The guard sits before the scheduler-step/checkpoint work so a NaN epoch doesn't step the scheduler. The atomic layer ([cache/atomic.py:70](../../src/modelfoundry/cache/atomic.py#L70)) writes the `FAILED` marker (`stage=training`) on the raised exception and leaves the temp dir intact.
- [x] **Tests.** A recipe forced to diverge raises with the epoch in the message and leaves a `FAILED` marker + the partial history; a normal converging run is unaffected (no false trip). → [test_pytorch_trainer.py](../../tests/integration/test_pytorch_trainer.py): `test_divergence_raises_with_epoch_and_preserves_partial_history` (huge LR `1e10` → `PluginError` matching `training diverged at epoch \d+` + partial `history.parquet`) + `test_converging_run_does_not_false_trip_divergence_guard` (control: 3 epochs run). [test_materialize_runner.py](../../tests/integration/test_materialize_runner.py): `test_training_divergence_writes_failed_marker_and_partial_history` (full runner → not promoted + `FAILED` marker `stage=training`/`error_class=PluginError` + partial history). Added a `learning_rate` knob to both files' `_recipe` helpers. Full CI gate green: ruff + ruff format --check (172) + mypy (172) + light (594 passed / 53 skipped) + smoke-pytorch.

**Version:** no bump (rides v0.19.0). Adds an error path only; no effect on a converging run's bytes.

---

### Story I.w: FR-13 visualization params [Done]

Complete the FR-13 visualization param surface — the spec promises `predictions_grid` params `n` / `splits` / `per_class` and `splits: list[str]` for `confusion_matrix` / `calibration_curve`, but [visualization_specs.py:38-45](../../src/modelfoundry/plugins/pytorch/visualization_specs.py#L38) carries only `max_items` + singular `split`. → Added the params to [visualization_specs.py](../../src/modelfoundry/plugins/pytorch/visualization_specs.py) + multi-split/per-class rendering in [visualizations.py](../../src/modelfoundry/plugins/pytorch/visualizations.py).

- [x] **`predictions_grid` params.** Add `n` (rename/alias of `max_items`), `splits`, and `per_class: bool = False`; implement split-filtering + per-class grouping in [`_predictions_grid`](../../src/modelfoundry/plugins/pytorch/visualizations.py#L166). Keep the labels-only fallback when the bound DR instance exposes no per-record images. → `PredictionsGridParams`: `n: int = 16` with `validation_alias=AliasChoices("n","max_items")` (legacy alias preserved), `splits: list[str] | None`, `per_class: bool = False`. Renderer filters by `splits` (`.isin`) and stable-sorts by `true_label` for per-class grouping; labels-only fallback unchanged.
- [x] **`confusion_matrix` / `calibration_curve` `splits`.** Accept `splits: list[str]` (default `Evaluation.splits`), rendering one figure over the listed splits; refactor `_pick_split` accordingly. → `SplitVizParams` gains `splits`; new `_pick_splits` (precedence: `splits` → `split` → evaluated splits **excluding the FR-12 `baseline` key** → `["val"]`). `confusion_matrix` renders one panel per split (`_draw_confusion` factored); `calibration_curve` overlays one line per split. **Default changed legacy first-split-only → all `Evaluation.splits`** per FR-13.
- [x] **Byte-neutrality (design constraint).** Add the params without perturbing the canonical bytes of any recipe authoring the current `split`/`max_items` form — alias-preserve, or migrate-the-corpus-with-justification; settle the approach in-story and assert byte-stability. → **Decided: alias-preserve (no corpus migration).** `VisualizationSpec` is `extra="allow"` and renderers read `model_extra`, so canonical bytes are the author's verbatim keys; new params are authored-only-when-used + `max_items` alias preserved ⇒ **canonical bytes unchanged**. Additionally kept the legacy single-split code paths **byte-identical** (`subplots(1,1)` reduces to the old figure; calibration single-split branch preserved) so existing instances' **output** PNG bytes are also unchanged. Asserted by `test_legacy_viz_form_does_not_bake_new_params` (verbatim dump, no `n`/`splits`/`per_class` baked).
- [x] **Tests.** Multi-split render; `per_class` grouping; byte-stability of an existing-form recipe. → [test_pytorch_visualizations.py](../../tests/unit/test_pytorch_visualizations.py): param-model accepts `n`/`max_items`/`splits`/`per_class`; `confusion_matrix`/`calibration_curve` multi-split render; `predictions_grid` per-class+splits render (deterministic); legacy-form byte-neutrality dump. Full CI gate green: ruff + ruff format --check (172) + mypy (172) + light (600 passed / 53 skipped) + smoke-pytorch.

**Version:** no bump (rides v0.19.0). New viz params are authored-only-when-used; no shipped recipe authors them. (FR-13's first-split→all-splits default change perturbs *output* PNG bytes only for a recipe authoring `confusion_matrix`/`calibration_curve` with no split on a multi-split eval — pre-prod, no shipped recipe does so; canonical bytes unchanged.)
...
### Story I.y: Doc sync, project-essentials append & release — owns the bump (→ v0.19.0) [Done]
...
- [x] **`features.md`** — fold in FR-12 (completed sklearn half + HF deferral note), FR-3 / FR-10 (now enforced), FR-13 / FR-9 (completed); add a new **FR-INFERENCE** documenting the shipped `Inference` block (`mode: point | mc_dropout`, `mc_samples`), the `.uncertainty` accessor, and the `predictive_entropy` / `mc_variance` persistence (divergence **D3**). Fix **D1** (viz `mode` `exploration` → `interactive`, all sites + FR-2 check 15) and **D2** (per-class metrics described as index-ordered `list`, not label-keyed dict). → Added **FR-28** (Stochastic inference / MC-dropout) + the `Inference` recipe-block entry; rewrote FR-12 behavior (sklearn `sklearn:<Class>` grammar, seeded fit-on-train, check-13-format, HF deferral) + its failure-mode edge case; updated FR-3 steps 1–2 (validate-first + refuse-on-hit, replacing the "return existing" wording) + the cache-hit edge case; extended check 6 (early-stopping **and** schedule monitor); D1 fixed at 5 sites (recipe-shape list, check 15, FR-13 ×2, inspect §); D2 fixed at the FR-22 `evaluation` accessor type + TR-9. (FR-10/FR-13 param text were already the accurate promise — now fulfilled.)
- [x] **`tech-spec.md`** — reflect the baseline resolver, the validate-first ordering, the divergence guard, and the expanded viz-param surface. → `evaluation.py` baseline line rewritten (sklearn resolver, not HF download); added a validate-first/refuse-on-hit note to the `MaterializeRunner` (FR-3) section; `trainer.py` gains the FR-10 divergence-guard note; `visualizations.py` gains the FR-13 param surface + byte-neutrality note; `inspect` flag-table `exploration` → `interactive`.
- [x] **`concept.md` / `README.md`** — touch only if scope wording shifts (baseline comparison is now real on the sklearn path); otherwise leave. → `concept.md`: `exploration` → `interactive` (×2) + `comparison_baseline_gap` rewritten (sklearn half shipped, HF deferred). **`README.md`: left** — its "baseline" usage is the model-swap quickstart (random/sklearn/pytorch), not the FR-12 comparison feature; no scope shift.
- [x] **`project-essentials.md`** — append new must-know facts: the baseline-comparison determinism rule (a fit-on-train baseline is a seeded stochastic source under the four invariants) and the doc-follows-code reconciliation precedent (D1/D2). → Added two sections: "**The FR-12 baseline comparison is a seeded stochastic source**" (`derive_seed(seed,"baseline")`, allowlist-not-dotted-path, check-13-format + runtime warn-and-skip, metrics-only persistence) and "**Doc-follows-code is the reconciliation default**" (D1/D2 + the FR-3 hit-semantics divergence as the precedent set).
- [x] **`## Future`** — record the deferred HF-pretrained baseline comparison as its own story. → Added "**FR-12 HuggingFace-pretrained baseline comparison (deferred half)**" (label-space alignment + `hf:<model-id>` grammar sketch + plan-doc anchor).
- [x] **Release** — own the single minor bump **→ v0.19.0**; `CHANGELOG.md` `## [0.19.0]` entry (feature-code reconciliation: sklearn baseline comparison + viz params + validate-first + divergence guard; **not cache-invalidating**). Full CI gate green: ruff + ruff format --check + mypy src tests + light + smoke-pytorch. → `_version.py` `0.18.0 → 0.19.0` (`modelfoundry.__version__` reports `0.19.0`); `CHANGELOG.md` `## [0.19.0] - 2026-06-24` entry. Full CI gate green: ruff + ruff format --check (172) + mypy (172) + light (603 passed / 53 skipped, incl. `test_release_metadata` CHANGELOG↔version guard at 0.19.0) + smoke-pytorch.
...
- **FR-12 HuggingFace-pretrained baseline comparison (deferred half)** — Subphase I-2 (Story I.t) shipped the **self-contained scikit-learn-estimator** half of FR-12 (`sklearn:<EstimatorClassName>`, fit-on-train, seeded). The **HuggingFace-pretrained-model** half is the design-heavy slice: it needs label-space alignment between the pretrained classifier's head and the bound instance's labels (head-swap / linear-probe / zero-shot policy) and leans on the deferred `[huggingface]` extra, so only the sklearn-class baseline can run in the pre-prod PyTorch-only install today. Likely shape: extend the `baseline_model_id` grammar with an `hf:<model-id>` form, resolve via the existing Encoder/LoRA path, and settle the label-alignment policy as its own design pass. Plan-doc anchor: `phase-i-subphase-2-feature-code-reconciliation-plan.md` § 5 item 1. A `plan_features`-shaped follow-up.
